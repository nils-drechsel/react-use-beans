...
class Deserialiser {
    ...
    deserialise(jsonString, expectedType) {
        let bean = JSON.parse(jsonString);
        if (bean && bean._t && this.deserialisers.has(bean._t)) {
            bean = this.deserialisers.get(bean._t).deserialise(bean);
        }
        if (expectedType) {
            if (bean._t !== expectedType) {
                throw new Error("Expected type " + expectedType + " but got " + bean._t + " in bean " + bean);
            }
        }
        return bean;
    }
